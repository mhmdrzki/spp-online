$(document).ready(function() {
    var popmedia = {
        createModal: function() {
            ...
            var mod_dialog = $('<div/>').addClass('modal-dialog').attr({style: 'width:800px'});
    ...
    var mmedia = {
        ...
        getData: function(tipe, from, start) {
            ...
            if (typeof(start) == 'undefined')
                start = 1;
            var tipe_call = 'admin/media_manager';
            if (tipe == 'album')
                tipe_call = 'admin/media_album';
        ...
        listing: function(tipe, from, data) {
            ...
            $(data.images).each(function(index, value) {
                if (typeof(value.info_ex) != 'undefined')
                    ...
                    sr_img = BASEURL + 'media/ico/icon-album.png';
        ...
        polhead: function(selected) {
            ...
            $(nav).append(images).append(upload).append(thefiles);
            $(dcenter).append(nav);
        ...
        formUpload: function(tipe) {
            ...
            var btn_file = $('<span/>').addClass('btn btn-default btn-file').append('<span class="fileinput-new"><i class="fa fa-search"></i> Select image</span><div class="fileinput-exists"><i class="fa fa-mail-reply"></i> Change</div>')
                    .append(inp_file);

            $(def).append(btn_file).append('<a href="#" class="btn btn-default fileinput-exists" data-dismiss="fileinput"><i class="fa fa-close"></i> Remove</a>');
            ...
            var btn_upload = $('<button/>').addClass('btn btn-warning').html('<i class="fa fa-check"></i> Upload');
...
//					$('.previewTarget').closest('.thumbnail').append(img_add);
        ...
        insertFile: function(from, data, sr_img) {
            ...
            var rdiv = $('<div/>'),
                    ...
                    a_view = $('<a/>').html('View').attr({href: BASEURL + "/index.php/admin/media_manager/viewapp/" + data.id, target: '__blank'});
    ...
    var imx = {
        ...
        uploadFiles: function(event) {
            ...
            $.ajax({
                url: BASEURL + 'index.php/admin/media_manager/uploadAjax?files',
                type: 'POST',
                data: data,
                cache: false,
                dataType: 'json',
        ...
        submitForm: function(event, data) {
            ...
            $.ajax({
                url: BASEURL + 'index.php/admin/media_manager/uploadAjax',
                type: 'POST',
                data: formData,
                cache: false,
                dataType: 'json',
